<?php
   ...
   session_start();
	...
	$query1=$con->prepare("SELECT a.*,b.id,b.job_position_name,b.job_location,c.id,c.company_name FROM job_application AS a LEFT JOIN jobs AS b ON a.job_code=b.job_code LEFT JOIN company AS c ON a.company_code=c.company_code WHERE a.student_code=?");
	...
	while($query1->fetch()){
		...
		<div class="row">
				...
				<div class="row">
					...
					<!--<div class="col-sm-1">
					<a type="button" name="viewApplicationBtn" id="viewApplicationBtn" value="view_application"  class="view_application action_icon" href="student_view_one_application.php?id='.$application_id.'">
					...
					<div class="col-sm-1"> 
						<a type="button" name="viewCompanyBtn" id="viewCompanyBtn" value="view_company" company_id='.$company_id.' class="view_company action_icon" data-toggle="modal" data-target="#student_company_view_modal">
                			<span class="fas fa-building view_company_icon action_icon" data-toggle="tooltip" title="View company"></span>
            			</a>
            		</div>
